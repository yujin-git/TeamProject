						...
						<div class="col-md-4 col-12">
							<div class="card">
								<div class="card-body">
									...
									<div class="row">
										<div class="col-12">
											<canvas id="ageNgender" width="400" height="400" style="z-index:1050;"></canvas>
										</div>
									</div>
									
									
									<ul class="list-style-none mb-0">
										<li class="mt-3"><i class="fas fa-circle text-primary font-10 mr-2"></i>
											<span class="text-muted">남성 회원수</span> <span
											class="text-dark float-right font-weight-medium">
											<c:set var="manCount" value="0"/>
										...
										<li class="mt-3"><i
											...
											class="text-dark float-right font-weight-medium">
											<c:set var="womanCount" value="0"/>
											<c:forEach items="${AgeNGender}" var="item">
								            	<c:if test="${item['GENDER'] =='F' }">
								            		<c:set var="womanCount" value="${womanCount + item['COUNT']}"/>
								            	</c:if>
							            	</c:forEach>
							            	${womanCount}
											</span>
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="d-flex align-items-start">
                                    <h4 class="card-title mb-0">메인카테고리별 회원수</h4>
                                    <div class="ml-auto">
                                ...
                                <ul class="list-inline text-center mt-4 mb-0">
                                    <li class="list-inline-item text-muted font-italic">총 모임 수: 
                                    	<c:set var="meetingCount" value="0"/>
											<c:forEach items="${CategoryNMeeting}" var="item">  	
								            		<c:set var="meetingCount" value="${meetingCount + item['COUNT']}"/>
							            	</c:forEach>
							            	${meetingCount}
                                    </li>
                                </ul>
...
<script>
	...
	/* categoryNmeeting */
	var ctx2 = document.getElementById('categoryNmeeting').getContext('2d');
	var categoryNmeeting = new Chart(ctx2, {
    type: 'bar',
    data: {
        labels: [
        	<c:forEach items="${CategoryNMeeting}" var="item">
