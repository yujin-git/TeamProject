			...
			<div class="d-flex align-items-center">
				<nav aria-label="breadcrumb">
					<ol class="breadcrumb m-0 p-0">
						...
						<li class="breadcrumb-item text-muted active" aria-current="page">정지 회원 조회</li>
					</ol>
				</nav>
			</div>
			<small>&nbsp;</small>
				...
				<div class="card-body">
					<div class="table-responsive">
						<table class="table table-bordered">
							...
							<tbody>
								<c:forEach items="${SuspendMemberList}" var="item">
									<tr>
										<td>${item['ENABLED']}</td>
										<td>${item['ID']}</td>
										<td>${item['NAME']}</td>
										...
										<td><fmt:formatDate pattern="YYYY-MM-dd"
												value="${item['SUSPEND']}" /></td>
										<td><c:if test="${item['ENABLED'] eq 0}">
												<a
													href='<c:url value="/Admin/Member/MemberSuspendRelease.do?id=${item['ID']}"/>'>
													해제</a>
