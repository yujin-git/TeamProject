...
<style type="text/css">
/* @font-face { */
/*     font-family: "binggrae_samancobold"; */
/*     src: url("resources/fonts/flaticon/font/TmoneyRoundWindExtraBold.ttf") format("truetype"); */
...
<body>
            ...
            <div class="filebox preview-image"
               ...
               <form id="uploadForm" action="<c:url value='/FileMessageSend.do'/>"
                  ...
                  <label for="input-file" id="input-file-label"> <img
                     src="<c:url value='/resources/images/file_icon_circle.png'/>"
   ...
   <script>
      $(document).ready(function() {
         ...
         loading +='<div id="loading" class="loading"></div><img id="loading_img" alt="loading" src="<c:url value="/resources/images/loading.gif"/>" style="margin-top: 7px "/>'
         loading +='</div></div></div>'
...
//                       // sendMessage(query)
            ...
            var chatting_msg = function() {
               ...
               //"ws://192.168.50.81:8080<c:url value='/chat-ws.do'/>");
            ...
            var appendMessage = function(msg, type) {
               console.log("채팅 msg_id:::",id);
               console.log("채팅 profileimg:::",profileimg);
               INDEX++;
               var str = "";
            ...
            var receiveMessage = function(e) { // e = message 이벤트 객체
               ...
               if (receiveData.substring(0, 4) == 'msg:') {
                  ...
                  var idNmsg = receiveData.substring(4).split(':')[1].substring(0);
                  msg_id = idNmsg.split('/')[0];
                  var idNmsg2 = idNmsg.split('/')[1];
                  profileimg = idNmsg2.split('-')[0];
                  msg = receiveData.substring(4).split('-')[1];
                  ...
                  msg_name = receiveData.substring(4).split(':')[0];
                  var idNmsg = receiveData.substring(4).split(':')[1].substring(0);
                  filename = idNmsg.split('/')[0];
                  var idNmsg2 = idNmsg.split('/')[1];
                  profileimg = idNmsg2.split('-')[0];
                  msg = receiveData.substring(4).split('-')[1];
                  console.log('MSG::',msg_name, msg_id, profileimg, msg);
            ...
            function sendMessage() {
               ...
               $.ajax({
            	   url:'<c:url value="getProfileImg.do"/>',
            	   data:{'id':$('#user_id').val()},
            	   dataType:'text',
            	   success:function(profileimg){
            		   // 서버로 메시지 전송
                       wsocket.send("msg:" + nickname + ':' + $('#user_id').val() + '/' + profileimg + '-' + $('#message').val());
            		   // DIV(대화영역)에 메시지 출력
            ...
            function sendFile(url, filename) {
               ...
               $.ajax({
             	   ...
             	   success:function(profileimg){
             		   // 서버로 메시지 전송
                       wsocket.send("img:" + nickname + ':' + $('#user_id').val() + '/' + profileimg + '-' + filename);
                       // DIV(대화영역)에 메시지 출력
